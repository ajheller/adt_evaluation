...
def plotly_image(
    ...
    trace = go.Heatmap(
        ...
        hoverinfo="text",
        text=(
            np.vectorize(lambda a, e, c: hovertext_format % (a, e, c))(
                T.az * 180 / np.pi, T.el * 180 / np.pi, XX
            )
        ).transpose(),
    )
    return trace
...
Angular_spread_trace = plotly_image(
    T,
    np.arccos(rEr) * 180.0 / np.pi,
    # Xmin=0.0,
    Xmax=45.0,
...
Derr_trace = plotly_image(
    T,
    180 / np.pi * np.arccos(np.sum(rEu * xyz0, 0)),
    Xmin=0.0,
    Xmax=15.0,
...
def speaker_hovertext(az, el, rr, id):
    def ht(az, el, rr, id):
        return "<B>%s</B><br>az: %.1f<br>el: %.1f<br> r: %.1f" % (id, az, el, rr)
...
speakers = go.Scatter(
    ...
    visible=True,
    showlegend=True,
    text=speaker_hovertext(
        spkr_az * 180 / np.pi, spkr_el * 180 / np.pi, spkr_rr, spkr_id
    ),
)

camera = dict(
    up=dict(x=1, y=0, z=0), center=dict(x=0, y=0, z=0), eye=dict(x=0, y=0, z=-2)
)
...
layout = go.Layout(
    title=name + "-%dH%dV" % (C["h_order"], C["v_order"]) + "<br>" + rE_trace["name"],
    showlegend=True,
    hovermode="closest",
...
fig = go.Figure(
    data=[rE_trace, Angular_spread_trace, Derr_trace, E_trace, speakers], layout=layout
)
...
if __name__ == "__main__":
    ...
    plotly.offline.plot(
        ...
        filename=os.path.join(
            out_dir,
            S["name"] + "-" + ("%dH%dV" % (C["h_order"], C["v_order"])) + "_rE.html",
        ),
    )
