...
from spherical_data import cart2sph, cart2sphz, sph2cart, spherical_cap, sphz2cart
...
def load_t_design_cart(file, four_pi=False):
    ...
    if four_pi:
        w *= 4 * pi
    return SphericalGrid(*t.T, w=w, name="T-design")
...
def load_t_design_sphz(file, four_pi=False):
    ...
    if four_pi:
        w *= 4 * pi
    return SphericalGrid(*SphD.sphz2cart(t[:, 1], t[:, 0]), w=w, name="T-design-sph")
...
def t_design240(*args, **kwargs) -> SphericalGrid:
    ...
    return load_t_design_cart(_data_dir / "des.3.240.21.txt", *args, **kwargs)


def t_design5200(*args, **kwargs) -> SphericalGrid:
    return load_t_design_sphz(
        _data_dir / "Design_5200_100_random.dat.txt", *args, **kwargs
    )
...
def az_el(resolution=180):
    ...
    return SphericalGrid(*sph2cart(az, el), w=w, name=f"AzEl: {len(u)}, {len(v)}")


# %% unit tests
def unit_tests():
    for t in (t_design240(), t_design5200()):
        s = [
            t.surface_integral(f)
            for f in (
                lambda x, y, z: 1,  # = 1
